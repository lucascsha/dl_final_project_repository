...
def get_speaker_feature(model, opts):
    ...
    with torch.no_grad():
        ...
        for speaker in tqdm(opts.speaker_list):
            os.makedirs('/home/ICT2000/yin/per_er/data/MSP-Podcast/{}'.format(opts.features), exist_ok=True)
            speaker_feature_path = '/home/ICT2000/yin/per_er/data/MSP-Podcast/{}/{}.npy'.format(opts.features, speaker)
...
def main(opts):
    # prediction TODO
    predictions = open('./checkpoints_minh/ptapt_{}_output_test_c.csv'.format(opts.label), 'r').readlines()
    opts.predictions = {}
    ...
    # data
    label_distribution = open('/home/ICT2000/yin/per_er/data/MSP-Podcast/per_labels/label_distribution.txt', 'r')
    ...
    opts.speaker_dict = {}
    csv_file = open('/home/ICT2000/yin/per_er/data/MSP-Podcast/per_labels/train.csv').readlines()[1:]
    csv_file += open('/home/ICT2000/yin/per_er/data/MSP-Podcast/per_labels/val.csv').readlines()[1:]
    csv_file += open('/home/ICT2000/yin/per_er/data/MSP-Podcast/per_labels/new_test_c.csv').readlines()[1:]
    for row in csv_file:
        speaker = row.split(',')[-2]
    ...
    if opts.backbone == 'base':
        opts.fairseq_ckpt_path = '/home/ICT2000/yin/per_er/checkpoints/hubert_base_ls960.pt'
    else:
        if opts.features == 'tapt':
            ...
            opts.fairseq_ckpt_path = '/home/ICT2000/yin/per_er/checkpoints/hubert_large_ll60k.pt'

    device = torch.device('cuda:0')
    model = FairseqAudioModel(opts, device).to(device)

    if opts.features == 'tapt_valence':
        ckpt_path = os.path.join('/home/ICT2000/yin/per_er/checkpoints_minh/valence.pt')
        model.load_state_dict(torch.load(ckpt_path), strict=False)
    elif opts.features == 'tapt_arousal':
        ckpt_path = os.path.join('/home/ICT2000/yin/per_er/checkpoints_minh/arousal.pt')
        model.load_state_dict(torch.load(ckpt_path), strict=False)
    elif opts.features == 'tapt_dominance':
        ckpt_path = os.path.join('/home/ICT2000/yin/per_er/checkpoints_minh/dominance.pt')
        model.load_state_dict(torch.load(ckpt_path), strict=False)
    elif opts.features == 'finetune_hubert_large':
        ckpt_path = os.path.join('/home/ICT2000/yin/per_er/checkpoints/MSP-Podcast/valence/hubert_large_100.pt')
        model.load_state_dict(torch.load(ckpt_path), strict=False)
